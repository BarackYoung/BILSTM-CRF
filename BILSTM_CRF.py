...
def pre_process_Data(TRAIN_DATA_PATH, TRAIN_DATA_PATH2):
    print("数据准备中。。。")
    ifp = open(TRAIN_DATA_PATH, encoding='UTF-8')
    ifp2 = open(TRAIN_DATA_PATH2, encoding='UTF-8')
    sentence_set = list()
    tag_set = list()
    sentence = ""
    tag = ""
    for line in ifp:
        # print()
        if (len(line)<4):
            sentence_set.append(list(sentence))
            tag_set.append(list(tag))
            sentence=""
            tag=""
        else:
            line.split()
...
def train(epochs):
    ...
    pre_data = pre_process_Data("dataset/dataset1/train.utf8", "dataset/dataset2/train.utf8")
    content = pre_data[0]
    label = pre_data[1]
    data = train_data(content, label)
    word_to_ix = load_obj("word_to_ix")
    for sentence, tags in data:
        for word in sentence:
            if word not in word_to_ix:
                word_to_ix[word] = len(word_to_ix)  # 单词映射，字到序号

    save_obj(word_to_ix, "word_to_ix")
    # model = BiLSTM_CRF(len(word_to_ix), tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM)
    model = torch.load('BILSTM_CRF.model')
    model.eval()
    ...
    # 保存模型
    torch.save(model, 'BILSTM_CRF.model')
    torch.save(model.state_dict(), 'dict.model')

def predict(sentence):
    net = torch.load('BILSTM_CRF.model')
    net.eval()
    word_to_ix = load_obj("word_to_ix")
...
if __name__ == '__main__':
    ...
    train(1)
    # print(predict("中国进出口银行与中国银行加强合作。"))